...
public class CargoTruck_CC_Damage
{
    ...
    public void Initialize(CargoTruck_CC_CarControllerV3 _carController) {
        ...
        damageData.GetParts(detachableParts);
                        ...
                        for (int k = 0; k < vertices.Length; k++) {
                            ...
                            if (distanceToVert <= damageRadius) {
                                ...
                                if (randomizeVertices > 0)
                                    rot *= Quaternion.Euler(randomizedVector);
    ...
    private void DamageWheel(float impulse) {
        ...
        for (int i = 0; i < wheels.Length; i++) {
            ...
            if (wheels[i].gameObject.activeSelf) {
                ...
                for (int i = 0; i < detachableParts.Length; i++) {
                    ...
                    if (detachableParts[i] != null && detachableParts[i].gameObject.activeSelf) {
                        ...
                        if (detachableParts[i].partCollider != null) {
                            ...
                            Vector3 closestPoint = detachableParts[i].partCollider.ClosestPointOnBounds(contactPoint.point);
                            float distance = Vector3.Distance(closestPoint, contactPoint.point);
                            float damage = impulse;
    ...
    public void OnCollision(Collision collision) {
        ...
        if (((1 << collision.gameObject.layer) & damageFilter) != 0) {
            ...
            if (impulse > 0f) {
                ...
                List<ContactPoint> cp = new List<ContactPoint>(collision.contactCount);

                //  First, we are getting all contact points.
                contactPoints = new ContactPoint[collision.contactCount];
                collision.GetContacts(contactPoints);

                float res = (damageResolution / 20f);
                res = (int)Mathf.Lerp(6f, 1f, res / 6f);

                for (int i = 0; i < contactPoints.Length; i++) {

                    if (i % res == 0)
                        cp.Add(contactPoints[i]);

                }

                contactPoints = cp.ToArray();
